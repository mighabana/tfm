...
def default_retriever(query):
    ...
    retrieval_output = [sentences[i] for i in indices[0]]
    
    cross_inp = [[query, c] for c in retrieval_output]
    cross_scores = crossencoder.predict(cross_inp)
    
    cross_values, cross_indices = torch.topk(torch.from_numpy(cross_scores), cross_k)
    
    context = [cross_inp[i][1] for i in cross_indices]
    
    return context, cross_values.numpy(), retrieval_output, values[0].numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--crossencoder_model', nargs='?', type=str, required=True)
    ...
    crossencoder = CrossEncoder(args.crossencoder_model)
    ...
    for idx, row in df.iterrows():
        ...
        evidence_content, crossencoder_score, retrieval_output, retrieval_score, = retrieval_chain.invoke(query)
        
        crossencoder_evidences.append(evidence_content)
        crossencoder_scores.append(crossencoder_score)
        
        retrieval_evidences.append(retrieval_output)
    ...
    output_df['crossencoder_evidences'] = crossencoder_evidences
    output_df['crossencoder_scores'] = crossencoder_scores

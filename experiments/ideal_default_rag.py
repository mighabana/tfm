...
if __name__ == "__main__":
    ...
    for idx, row in df.iterrows():
        ...
        expected_evidence_len = len(row['evidences'])
        evidence_content_len = len(evidence_content)
        evidence = row['evidences'][:6] if expected_evidence_len >= 6 else list(row['evidences'])[:expected_evidence_len] + list(evidence_content)[:evidence_content_len - 6]

        
        retrieval_scores.append(retrieval_score)
        ideal_evidences.append(evidence)
        ...
        try:
            answer = default_rag_chain.invoke({'context': evidence, 'question': query})
        except:
    ...
    output_df['ideal_evidences'] = ideal_evidences
